...
import { storeNewFavorite, getStorageItemsSize, itemExistsInFavs, clearStorage } from '../API/StorageController';
...
export default function Details({ route }) {
  ...
  const add2Favs = (api_link: String) => {
    console.log("Antes " + getStorageItemsSize())
    setModalVisible(false)
    result.current = storeNewFavorite(api_link).result;
    console.log("Despues : " + getStorageItemsSize() + " " + result);
      ...
      <View style={style.modalContainer}>
        <View style={{flex:1, justifyContent:'center'}}>
            ...
            <View style={{flex:1, flexDirection:'row', alignItems:'center'}}>
              <TouchableOpacity style={style.buttonCancel} onPress={()=>{setModalResultVisible(false)}} >
                <Text>Ok</Text>
              </TouchableOpacity>
            </View>
        </View>
  ...
  return (
    ...
    <Modal transparent={true} visible={modalRepeatedVisible}>
      <View style={style.modalContainer}>
        <View style={{flex:1, justifyContent:'center'}}>
            <Text style={style.addToFavText}>Artwork already in favorites</Text>
            <View style={{flex:1, flexDirection:'row', alignItems:'center'}}>
    ...
    <ScrollView style={{ flex: 1, backgroundColor: Config.COLOR_NEGRO }}>
      ...
      <View style={style.title}>
        ...
        <TouchableOpacity style={{ 
          ...
          opacity: itemExistsInFavs(info.api_link) ? 1  : 0.5 ,
          }}>
          <Icon 
            name="heart" 
            size={30} 
            color="black" 
            style={{
              color: itemExistsInFavs(info.api_link) ? Config.COLOR_ROJO_FAV : Config.COLOR_NARANJA,
            }} 
            onPress={()=>{
